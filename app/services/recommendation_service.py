...
from app.models import User, Book, Recommendation, UserPreferences
...
def train_recommendation_model(db: Session):
    ...
    # Prepare the data
    book_data = [
        f"{book.genre} {book.author} {book.summary} {book.content}"
        for book in books
    ]
...
def get_recommendations(db: Session, user_preferences: UserPreferences):
    ...
    preference_text = f"{user_preferences.preferred_genres} {user_preferences.preferred_authors}"
...
def precompute_recommendations_for_all_users(db: Session):
    ...
    for user in users:
        ...
        existing_recommendation = db.query(Recommendation).filter(Recommendation.user_id == user.id).first()
...
def compute_recommendation(db: Session, user_id: int):
    ...
    existing_recommendation = db.query(Recommendation).filter(Recommendation.user_id == user_id).first()

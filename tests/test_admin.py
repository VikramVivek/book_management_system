...
def test_admin_update_user(client, admin_token):
    ...
    headers = {"Authorization": f"Bearer {admin_token}"}
    response = client.put(f"/admin/users/{user_id}", headers=headers, json={
...
def test_admin_update_review(client, admin_token, create_test_review):
    ...
    response = client.put(f"/admin/reviews/{review_id}", json={
        "review_text": "Admin has updated this review.",
        "rating": 2
    }, headers=headers)
    assert response.status_code == 200
    data = response.json()
